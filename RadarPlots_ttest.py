...
file_name = glob(rep + "/pval_CC_3*" + name_modality + "*.nii.gz")
file_name.sort()
#name_patient = [name.split("CC_3_")[1].split("_")[0] for name in file_name]
#nb_patient = len(file_name)

ROI_name = ['AF_left', 'AF_right', 'ATR_left', 'ATR_right', 'CC_1', 'CC_2', 'CC_3', 'CC_4',
...
IMA = ima.get_data()
aff = ima.get_affine()
Ttest_all = np.zeros(ima.shape)

###########
############ Estimate the number of damaged voxels with Bonferroni correction
############

data = np.zeros([ len(ROI_name), 4])

for i in range(len(ROI_name)):
        ima_name = rep +"pval_" + ROI_name[i] + "_" + name_modality + "_80.nii.gz"
        print(ima_name)
        ...
        nib.save(IMA_nib, "ttest_mask_" + ROI_name[i] + "_" + name_modality + ".nii.gz")
        ...
        data[ i, 0] = len(np.where(IMA_bin)[0]) / size_track
        data[ i, 1] = B
        if B > 1:
        #To estimate not the percentage but the number of voxels, remove the /size_track
            data[ i, 2] = np.max(size_les) / size_track
            data[ i, 3] = len(np.where(np.array(size_les) > 5)[0])
        else:
            data[ i, 2] = 0
            data[ i, 3] = 0
    # create a dataframe to save the quantitative measures
        df = pd.DataFrame(data=data[ :, :], columns=['all_voxels', 'nb lesions', 'max lesions', 'nb_lesions_large'])
        df.index = ROI_name
        df.to_excel(rep + "xls_" + name_modality + "_nocorrection.xls")
...
plt.show()
#Here we give the name all voxels lesion since we regard data[:,0]
plt.savefig('ttest_radarplot_allvoxels_lesion_' + name_modality + '_nocorrection.png')
plt.close('all')
